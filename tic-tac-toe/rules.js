...
function play() {
	if(started){
		moveElement = document.getElementById("move_text_id")
		chosen = false
		for(i = 0; i < table_ids.length; i++){
			currMove = moveElement.value.toUpperCase();
			console.log(currMove)
			//Using regex command to get rid of whitespace from documentation
			currMove = currMove.replace(/ /g, "");
			if(currMove == table_ids[i]){
				chosen = true
				break;
			}
		}
		if(chosen == false){
			...
			cell = document.getElementById(moveElement.value.toUpperCase())
			if(turn == 1 && ((cell.innerHTML != "X")&&(cell.innerHTML != "O"))){
				cell.innerHTML = "X"
				board_state[i] = Number(turn)
				toggle_move()
				...
				alert(document.getElementById("player1_id").value + " is already here!")
			}
			else if (cell.innerHTML == "O"){
				alert(document.getElementById("player2_id").value + " is already here!")
			}
		}
		document.getElementById("move_text_id").value = ""
		winner = ["O","X"]
		...
		&&(board_state[0]!=-1)){
			alert("Winner is: " + winner[board_state[0]])
			reset_play()
		}
		else if((board_state[1]==board_state[4]&&board_state[4]==board_state[7])&&(board_state[1]!=-1)){
			alert("Winner is: " + winner[board_state[1]])
			reset_play()
		}
		else if(((board_state[2]==board_state[4]&&board_state[4]==board_state[6])
			||(board_state[2]==board_state[5]&&board_state[5]==board_state[8]))&&(board_state[2]!=-1)){
			alert("Winner is: " + winner[board_state[2]])
			reset_play()
		}
		else if((board_state[3]==board_state[4]&&board_state[4]==board_state[5])&&(board_state[3]!=-1)){
			alert("Winner is: " + winner[board_state[3]])
			reset_play()
		}
		else if((board_state[6]==board_state[7]&&board_state[7]==board_state[8])&&(board_state[6]!=-1)){
			alert("Winner is: " + winner[board_state[6]])
			reset_play()
		}
	}
	else {
